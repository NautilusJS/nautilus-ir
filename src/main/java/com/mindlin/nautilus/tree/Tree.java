...
import com.mindlin.nautilus.tree.type.OptionalTypeTree;
import com.mindlin.nautilus.tree.type.ParenthesizedTypeTree;
import com.mindlin.nautilus.tree.type.RestTypeTree;
...
import com.mindlin.nautilus.tree.type.TypePredicateTree;
...
public interface Tree {
	public static enum Kind {
		// ===== Data structures =====
		// Program
		COMPILATION_BUNDLE(CompilationBundleTree.class),
		...
		AWAIT(AwaitExpressionTree.class),
		YIELD(YieldTree.class),
		YIELD_GENERATOR(YieldTree.class),//TODO: merge with yield?
		...
		ENUM_MEMBER(EnumMemberTree.class),
		MODULE_DECLARATION,
		...
		// Templates
		TAGGED_TEMPLATE(TaggedTemplateLiteralTree.class),
		TEMPLATE_LITERAL(TemplateLiteralTree.class),
		TEMPLATE_ELEMENT(TemplateElementTree.class),
		
		// Unary operators
		UNARY_PLUS(PrefixUnaryExpressionTree.class),
		UNARY_MINUS(PrefixUnaryExpressionTree.class),
		TYPEOF(TypeOfExpression.class),
		VOID(VoidExpressionTree.class),
		DELETE(DeleteExpression.class),
		
		// Update expressions
		POSTFIX_DECREMENT(PostfixUnaryExpressionTree.class),
		POSTFIX_INCREMENT(PostfixUnaryExpressionTree.class),
		PREFIX_DECREMENT(PrefixUnaryExpressionTree.class),
		PREFIX_INCREMENT(PrefixUnaryExpressionTree.class),
		...
		BITWISE_NOT(PrefixUnaryExpressionTree.class),
		...
		LOGICAL_NOT(PrefixUnaryExpressionTree.class),
		...
		PROPERTY_ACCESS(PropertyAccessTree.class),
		...
		// Array stuff
		SPREAD(SpreadElementTree.class),
		...
		QUALIFIED_NAME(QualifiedNameTree.class),
		
		// ===== Properties =====
		// These are used for object literals, classes, interfaces, etc.
		
		// Signatures
		PROPERTY_SIGNATURE(PropertySignatureTree.class),
		...
		SEMICOLON_CLASS_ELEMENT(SemicolonClassElementTree.class),
		...
		TYPE_ASSERTION(TypeAssertionTree.class),
		...
		Kind(Class<? extends Tree> clazz) {
			...
			this.expr = this.litr || ExpressionTree.class.isAssignableFrom(clazz);//literals are expressions
	...
	/**
	 * Marks a class as should be the implementation for some {@link Kind}.
	 * 
	 * @author mailmindlin
	 */
	...
	@Target(ElementType.TYPE)
	@Retention(RetentionPolicy.SOURCE)
	public static @interface ADT {
		String name() default "__infer__";
	}
	
	/**
	 * Marks an interface as a non-marker.
	 * 
	 * @author mailmindlin
	 */
	@Target(ElementType.TYPE)
	@Retention(RetentionPolicy.SOURCE)
	public static @interface NoImpl {
	...
	public static <@NonNull T extends Tree> boolean equivalentToUnordered(Collection<? extends T> a, Collection<? extends T> b) {
		if (a == b)
			return true;
		if (a == null || b == null || a.size() != b.size())
			return false;
		
		Map<Tree.Kind, Set<T>> lut = a.stream().collect(Collectors.groupingBy(Tree::getKind, Collectors.toSet()));
		
		outer:
		for (T vB : b) {
			Set<T> candidates = lut.getOrDefault(vB.getKind(), null);
			if (candidates == null)
				return false;
			
			if (candidates.remove(vB))
				continue;
			
			for (Iterator<? extends T> i = candidates.iterator(); i.hasNext(); ) {
				if (!Tree.equivalentTo(i.next(), vB))
					continue;
				i.remove();
				continue outer;
			}
...
//	<T> T setProperty(TreeProperty<T> key, T value);
	
//	<T> T getProperty(TreeProperty<T> key);
	
	@NonNull SourceRange getRange();
	...
	<R, D> R accept(@NonNull TreeVisitor<R, D> visitor, D data);
